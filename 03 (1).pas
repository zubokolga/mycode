...
constructor List.copy(arg_r : list_ptr);
	var
    temp_p1, temp_p2 : node_ptr;
  begin
    if arg_r = nil then
    exit;
    temp_p1 := arg_r^.intro_node_p;
    self.init(temp_p1^.data);
    temp_p2 := self.intro_node_p;
    while temp_p1^.next <> nil do begin
      temp_p1 := temp_p1^.next;
      self.push_back(temp_p1^.data);
